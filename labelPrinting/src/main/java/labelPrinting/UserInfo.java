package labelPrinting;

public class UserInfo {
	
	
	public static final String userID="UANGOTEST";
	public static final String password="UANG06demo";
	public static final String locationID="TESTSITEUANGOCOLTUSM";
	public static final String workstationID="DESKTOP-PNUDC2N";
	public static final String shippingAgentID="UANGOCOLTUSM";
	public static final String receivingAgentID="USPSERVICUSD";

}
